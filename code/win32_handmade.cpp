...
internal void CatStrings(size_t SourceACount, const char *SourceA, size_t SourceBCount, const char *SourceB, size_t DestCount, char *Dest) {
...
internal void Win32GetEXEFileName(win32_state *State) {
    ...
    DWORD SizeOfFilename = GetModuleFileNameA(nullptr, State->EXEFileName, sizeof(State->EXEFileName));
...
internal void Win32BuildEXEPathFileName(win32_state *State, const char *FileName, int DestCount, char *Dest) {
    CatStrings(
        State->OnePastLastEXEFileNameSlash - State->EXEFileName,
        State->EXEFileName,
...
internal void Win32DisplayBufferInWindow(win32_offscreen_buffer *Buffer, HDC DeviceContext, int WindowWidth, int WindowHeight) {
    // NOT(Beau): For prototyping purposes, we're going to always blit 1-to-1 pixels
    // to make sure we don't introduce artifacts with stretching while we are learning to code the renderer!
    StretchDIBits(
        DeviceContext,
...
internal void Win32BeginRecordingInput(win32_state *State, int InputRecordingIndex) {
    // TODO(Beau): Lazily write the giant memory block and use a memory copy instead?
    ...
    DWORD BytesToWrite = static_cast<DWORD>(State->TotalSize);
    Assert(State->TotalSize == BytesToWrite);
    DWORD BytesWritten;
    WriteFile(State->RecordingHandle, State->GameMemoryBlock, BytesToWrite, &BytesWritten, nullptr);
}
...
internal void Win32BeginInputPlayBack(win32_state *State, int InputPlayingIndex) {
    ...
    DWORD BytesToRead = static_cast<DWORD>(State->TotalSize);
    Assert(State->TotalSize == BytesToRead);
    DWORD BytesRead;
    ReadFile(State->PlayBackHandle, State->GameMemoryBlock, BytesToRead, &BytesRead, nullptr);
}
...
internal void Win32RecordInput(win32_state *State, game_input *NewInput) {
    DWORD BytesWritten;
    WriteFile(State->RecordingHandle, NewInput, sizeof(*NewInput), &BytesWritten, nullptr);
}

internal void Win32PlayBackInput(win32_state *State, game_input *NewInput) {
    DWORD BytesRead;
    if (ReadFile(State->PlayBackHandle, NewInput, sizeof(*NewInput), &BytesRead, nullptr)) {
        if (BytesRead == 0) {
            // NOTE(Beau): We've hit the end of the stream, go back to the beginning
            int PlayingIndex = State->InputPlayingIndex;
            Win32EndInputPlayBack(State);
            Win32BeginInputPlayBack(State, PlayingIndex);
            ReadFile(State->PlayBackHandle, NewInput, sizeof(*NewInput), &BytesRead, nullptr);
        }
    }
}
    ...
    while (PeekMessage(&Message, nullptr, 0, 0, PM_REMOVE)) {
        switch (Message.message) {
            ...
            case WM_KEYUP: {
                ...
                // NOTE(Beau): Since we are comparing WasDown to IsDown, we MUST use == and !- to convert these bit tests to actual 0 or 1 values
...
#define GameUpdateHz (MonitorRefreshHz / 2)
    ...
    if (RegisterClass(&WindowClass)) {
        ...
        if (Window) {
            ...
            GameMemory.PermanentStorageSize = Megabytes(static_cast<uint64>(64));
            GameMemory.TransientStorageSize = Gigabytes(static_cast<uint64>(1));
            GameMemory.DEBUGPlatformFreeFileMemory = DEBUGPlatformFreeFileMemory;
            GameMemory.DEBUGPlatformReadEntireFile = DEBUGPlatformReadEntireFile;
            GameMemory.DEBUGPlatformWriteEntireFile = DEBUGPlatformWriteEntireFile;

            // TODO(Beau): Handle various memory footprints (USING SYSTEM METRICS)
            // TODO(Beau): Use MEM_LARGE_PAGES and call adjust token privileges when not on Windows XP
            Win32State.TotalSize = GameMemory.PermanentStorageSize + GameMemory.TransientStorageSize;
            Win32State.GameMemoryBlock = VirtualAlloc(
                BaseAddress,
                static_cast<size_t>(Win32State.TotalSize),
